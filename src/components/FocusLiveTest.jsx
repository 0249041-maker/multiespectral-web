...
/** Distinta del flujo «Cámara multiespectral» para no mezclar borradores en localStorage. */
const CALIBRATION_WS_STORAGE_KEY = "focus-live-calibration-ws-url";
const LEGACY_WS_STORAGE_KEY = "focus-live-ws-url";
 ...
 *
 * @param {{ seedWsUrl?: string; autoStart?: boolean; compact?: boolean }} [props]
 */
export default function FocusLiveTest({
  ...
  const [wsUrl, setWsUrl] = useState(() => {
    const seed =
      typeof seedWsUrl === "string" && seedWsUrl.trim().length > 0
        ? seedWsUrl.trim()
        : "";
    if (seed) return seed;
    try {
      const saved =
        window.localStorage.getItem(CALIBRATION_WS_STORAGE_KEY) ||
        window.localStorage.getItem(LEGACY_WS_STORAGE_KEY);
  ...
  useEffect(() => {
    if (typeof seedWsUrl !== "string" || !seedWsUrl.trim()) return;
    setWsUrl(seedWsUrl.trim());
  }, [seedWsUrl]);

  const persistUrl = (url) => {
    try {
      window.localStorage.setItem(CALIBRATION_WS_STORAGE_KEY, url);
    } catch {
      // ignore
    }
  };
  ...
  useEffect(() => {
    ...
    const trimmed = wsUrl.trim();
    if (!trimmed.startsWith("ws://") && !trimmed.startsWith("wss://")) return;
    autoStartedRef.current = true;
    const id = requestAnimationFrame(() => {
      startStream();
    });
    return () => cancelAnimationFrame(id);
    // eslint-disable-next-line react-hooks/exhaustive-deps -- arranque único con URL ya sembrada
  }, [autoStart, wsUrl]);
  ...
  return (
    ...
    >
      <h2 className="text-lg font-semibold text-slate-900">
        {compact ? "Vista en vivo" : "Vista en vivo · calibración de enfoque"}
      </h2>
      ...
      {!compact ? (
        ...
        <p className="mt-1 text-xs text-slate-500">
          Stream JPEG por WebSocket (HTTPS → usa <code className="rounded bg-slate-100 px-1">wss://</code>).
        </p>
      )}
      ...
      {compact ? (
        <div className="mt-3 space-y-3">
          ...
          <details className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-xs text-slate-700">
            <summary className="cursor-pointer font-medium text-slate-800">
              URL del túnel (Cloudflare / LAN)
            </summary>
            <div className="mt-2 flex flex-col gap-2 sm:flex-row sm:items-end">
              <div className="flex min-w-0 flex-1 flex-col gap-1">
                <label htmlFor="focus-live-ws-compact" className="text-sm font-medium text-slate-800">
                  WebSocket URL
                </label>
                <input
                  id="focus-live-ws-compact"
                  type="text"
                  value={wsUrl}
